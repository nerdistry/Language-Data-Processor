...
def build_file_path(file_name: str) -> str:
    """The function takes a file name as input and returns the path by
    joining it with a directory (dataset_dir) using os.path.join()."""
    return os.path.join(dataset_dir, file_name)

def read_jsonl_file(path: str) -> pd.DataFrame:
    """The function reads a JSON Lines file (path) into a pandas
    DataFrame (df) and returns that DataFrame."""
...
def merge_with_english(english_df: pd.DataFrame, other_df: pd.DataFrame) -> pd.DataFrame:
    ...
    df = df[["id", "utterance", "utt_translation", "annot_utt_translation",
             "annot_utt", "partition"]]
    return df

def generate_xlsx_file(file: str) -> None:
    """The function processes a JSON Lines file, merges it with
    an English dataset, and saves the merged data into an Excel file."""
    sys.stdout.write(next(spinner))
    sys.stdout.flush()
    time.sleep(0.15)
    sys.stdout.write('\b')
    ...
    english_jsonl = pd.read_json(f"{dataset_dir}/{english_dataset}",
                                 lines=True)
...
def process_all_files(data_dir: str):
    """The function processes all files in a directory concurrently,
     using multiprocessing, by generating Excel files for each JSON Lines file in the directory."""
